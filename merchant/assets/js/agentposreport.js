...
$(document).ready(function() {
  ...
  //GET api/Invoice/Business/Agent?BusinessId={BusinessId}&AgentID={AgentID}&Status={Status}&QueryAmount={QueryAmount}&Month={Month}
  
  //GET All PAID INVOICES
  $("#invoices-get-paid").click(function(){
  $.LoadingOverlay("show");//show ajax loader
  $.ajax({
    url: apiBaseUrl + "Invoice/Business/Agent?BusinessId="+businessId+"&AgentID="+businessAgentId+"&Status=PAID&QueryAmount=0&Month=0",
    crossDomain: true,
    dataType: "json",
    method: "GET",
  ...
  $.LoadingOverlay("show");//show ajax loader
   $.ajax({
    url: apiBaseUrl + "Invoice/Business/Agent?BusinessId="+businessId+"&AgentID="+businessAgentId+"&QueryAmount=0&Month=0",
    crossDomain: true,
    dataType: "json",
    method: "GET",
    beforeSend: function(xhr) {
  ...
  $.LoadingOverlay("show");//show ajax loader
    $.ajax({
     url: apiBaseUrl + "Invoice/Business/Agent?BusinessId="+businessId+"&AgentID="+businessAgentId+"&Status=Pending&QueryAmount=0&Month=0",
     crossDomain: true,
     dataType: "json",
     method: "GET",
     beforeSend: function(xhr) {
